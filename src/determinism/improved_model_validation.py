...
def completed_keys(rows: list[dict]) -> set[tuple[str, int, str]]:
    return {
        (row["stage"], int(row["candidate_rank"]), row["composition"])
        for row in rows
        if row.get("status") == "ok"
    }


def configure_fixed_model(model, args: argparse.Namespace) -> None:
...
def build_summary(rows: list[dict], args: argparse.Namespace) -> dict:
    logits_rows = [
        row for row in rows
        if row["stage"] == "logits" and row.get("status") == "ok"
    ]
    generation_rows = [
        row for row in rows
        if row["stage"] == "generation" and row.get("status") == "ok"
    ]
    batch_size_rows = [
        row for row in rows
        if row["stage"] == "batch_size" and row.get("status") == "ok"
    ]
    return {
        ...
        "logits_summary": {
            "tested_cases": len(logits_rows),
            "bitwise_equal_cases": sum(
                str(row["logits_bitwise_equal"]).lower() == "true"
                for row in logits_rows
            ),
            "nonzero_difference_cases": sum(
                float(row["max_abs_diff"]) != 0.0 for row in logits_rows
            ),
            "top1_changed_cases": sum(
                str(row["top1_changed"]).lower() == "true" for row in logits_rows
            ),
            "top5_changed_cases": sum(
                str(row["top5_changed"]).lower() == "true" for row in logits_rows
            ),
            "maximum_absolute_difference": max(
                (float(row["max_abs_diff"]) for row in logits_rows),
                default=None,
            ),
        },
        "generation_summary": {
...
def main() -> None:
    ...
    configure_fixed_model(model, args)
    ...
    for rank, candidate in enumerate(candidates[:args.logits_candidates], start=1):
        pending_compositions = [
            ...
            if ("logits", rank, composition) not in done
        ]
        ...
        print(
            f"logits candidate {rank}/{args.logits_candidates} completed",
            flush=True,
        )
    ...
    pending_generation = [
        ...
        if ("generation", args.generation_rank, composition) not in done
    ]
