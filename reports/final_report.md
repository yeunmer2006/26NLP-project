...
但尚未替换 Matmul/Linear，因此不能宣称整个模型已经端到端 batch invariant。
...
目前还没有替换 Linear/Matmul，因此不能宣称模型端已经完全 batch invariant。
...
| 模型级 Batch Invariance 解决 | 部分完成 | fixed-tree RMSNorm 与 `flash_attn_2_bi` 已实现，Linear/Matmul 未替换 |
...
- 模型级解决方案已覆盖 RMSNorm 和 attention 归约，Linear/Matmul 和 serving 系统仍是后续工作。

## 9. 后续工作

当前课程项目主线已经闭环。继续扩展时，应优先做三件事：

1. **实现 batch-invariant Linear/Matmul。** 当前 fixed-tree 只覆盖 RMSNorm，
   `flash_attn_2_bi` 只覆盖 attention online-softmax 相关归约，尚未消除 attention
   projection、MLP 和 LM head 中矩阵乘带来的 batch dependence。
...
KV split 和 online-softmax 归约顺序，但还不能覆盖 Linear/Matmul。
...
7. **修复尝试**：展示 fixed-tree RMSNorm、`flash_attn_2_bi` attention 和
   `attention_invariance.png`，明确 Matmul/Linear 仍未覆盖。
